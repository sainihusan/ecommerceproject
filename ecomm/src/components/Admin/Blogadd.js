...
export default function Blogadd() {
    ...
    formDataToSend.append("blogImg", formData.propertyImg);
        try {
            if (isEdit) {
                ...
                await axios.put(`http://localhost:9000/blog/update/${formData.id}`, formDataToSend);
            } else {

                await axios.post('http://localhost:9000/blog/add', formDataToSend);
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:9000/blog/delete/${id}`);
            fetchData();
        } catch (error) {
                        ...
                        <TableBody>
                            {data.map((row,index) => (
                                <TableRow key={row.id}>
                                    ...
                                    <TableCell>
                                     <img width={55} src={`http://localhost:9000/uploads/${row.image_url}`} />
                                    </TableCell>
